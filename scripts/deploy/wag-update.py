...
WAG_CONF_ALIASES = 'wagConfAliases'
WAG_CONF_CUSTOM = 'wagConfCustom'
...
class Configuration:
    ...
    def __init__(self, data: Dict[str, Any], skip_remote_checks=False):
        ...
        self._kc_aliases: Dict[str, str] = data.get(WAG_CONF_ALIASES, {})
        self._kc_custom: List[str] = data.get(WAG_CONF_CUSTOM, [])
    ...
    def wag_conf_files(self) -> List[str]:
        return [
            ...
            for k in chain(WAG_CONF_FILES, self._kc_custom)
        ]
...
if __name__ == '__main__':
    ...
    argp = argparse.ArgumentParser(
        ...
        add_help=False,
        description=dedent('''\
            A simple WaG deployment script.

            Note: only core configuration files are installed by default:

            [ {} ]

            To add more files, please configure "wagConfCustom" item
            in your deployment config file.'''.format(', '.join(WAG_CONF_FILES))))
