...
describe('QueryFormModel', function () {
    ...
    ): TestModelWrapper<QueryFormModel, QueryFormModelState> {
        const initialQueryMatches = [
            [
                ...
                },
                {
                    word: 'test',
                    lemma: 'test',
                    upos: [{ value: UPoSValues.VERB, label: 'verb' }],
                    pos: [{ value: PoSValues.VERB, label: 'verb' }],
                    isCurrent: false,
                },
            ],
        ] as QueryMatch[][];
    ...
    describe('query input', function () {
            ...
            setupModel().checkState(
                {
                    ...
                    payload: { queryIdx: 0, value: 'query changed' },
                },
        ...
        describe('cmp query input', function () {
            it('adds input', function (done) {
                setupModel({ maxCmpQueries: 5 }).checkState(
            ...
            it('exceedes max number of inputs', function (done) {
                setupModel({ maxCmpQueries: 1 }).checkState(
                    { name: Actions.AddCmpQueryInput.name },
                    Actions.AddCmpQueryInput.name,
                    (state, appServices) => {
    ...
    describe('query match', function () {
        it('shows modal', function (done) {
            setupModel({ lemmaSelectorModalVisible: false }).checkState(
        ...
        it('hides modal', function (done) {
            setupModel({ lemmaSelectorModalVisible: true }).checkState(
        ...
        it('selects match', function (done) {
            setupModel({ modalSelections: [0] }).checkState(
                {
                    name: Actions.SelectModalQueryMatch.name,
                    payload: { queryIdx: 0, variantIdx: 1 },
                },
