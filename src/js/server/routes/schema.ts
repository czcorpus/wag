...
import { createParentWagLink, importQueryRequest, markMatch, mkRuntimeClientConf, QueryActionArgs } from './main.js';
import { TileConf } from '../../page/tile.js';

const tileConf: {[name:string]:TileConf} = {
    conc: {
...
const layoutConf: LayoutsConfig = {
    single: {
        groups: [
            {
                tiles: [
...
const responseConf: Array<Array<any>> = [
    [
        {
            concSize: 10,
            ipm: 38.1,
...
export function schemaPage({
    ...
    const appServices = new AppServices({
        ...
        dataStreaming: new DataStreamingMock(null, [], undefined, 1000, undefined, responseConf),
    ...
    rxOf({
        ...
        queryType,
        queries: [{word: 'test', lemma: 'test', pos: ['N']}],
        answerMode,
    } as UserConf).pipe(
        concatMap(
