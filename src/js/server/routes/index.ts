...
export const wdgRouter = (services: Services) => (app: Express) => {
  ...
  app.post(HTTPAction.TELEMETRY, (req, res, next) => {
    ...
    rxOf(
      ...(services.telemetryDB
        ? (req.body["telemetry"] as Array<TelemetryAction>)
        : []),
    )
      .pipe(
        concatMap(
        ...
        tap(() => services.telemetryDB.run("COMMIT")),
      )
      .subscribe({
        next: (total) => {
      ...
      if (db === undefined) {
        observer.error(
          new ServerHTTPRequestError(
            HTTP.Status.BadRequest,
            `Frequency database for [${queryDomain}] not defined`,
          ),
        );
      } else {
        observer.next(db);
        ...
        concatMap((db) => {
          return db.findQueryMatches(
            ...
            getQueryValue(
              req,
              "mainPosAttr",
              services.clientConf.layouts[queryDomain][QueryType.SINGLE_QUERY]
                .mainPosAttr,
          ...
          res.send(
            JSON.stringify({
              resultURL: appServices.createActionUrl(
                `${HTTPAction.SEARCH}${conf.query1Domain}/${conf.queries[0].word}`,
              ),
              error: null,
            }),
          );
    ...
    new Observable<{
      ...
      if (isNaN(parseInt(getQueryValue(req, "srchRange")[0]))) {
        observer.error(
          new ServerHTTPRequestError(
            HTTP.Status.BadRequest,
            `Invalid range provided, srchRange = ${req.query.srchRange}`,
          ),
        );
      } else if (
        services.db.getDatabase(QueryType.SINGLE_QUERY, queryDomain) ===
        undefined
      ) {
        observer.error(
          new ServerHTTPRequestError(
      ...
      .pipe(
        ...
        map((data) =>
          data.sort((v1: QueryMatch, v2: QueryMatch) => {
            if (v1.arf !== v2.arf) {
  ...
  app.get(HTTPAction.WORD_FORMS, (req, res) => {
    ...
    const freqDb = services.db.getDatabase(
      QueryType.SINGLE_QUERY,
      getQueryValue(req, "domain")[0],
    );
    ...
    new Observable<{
      ...
      .pipe(
        concatMap((args) =>
          freqDb.getWordForms(appServices, args.lemma, args.pos, args.posAttr),
        ),
      )
      .subscribe({
        next: (data) => {
  ...
  app.get(HTTPAction.SOURCE_INFO, (req, res) => {
    ...
    const queryType = importQueryTypeString(
      getQueryValue(req, "queryType")[0],
      QueryType.SINGLE_QUERY,
    );
    const queryDomain = getQueryValue(req, "domain")[0];
