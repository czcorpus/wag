...
function mkRuntimeClientConf({
  ...
  return forkJoin([
    forkJoin(
      List.map(
        (item) =>
          appServices.importExternalText(item.contents, loadFile).pipe(
    ...
    map(([tiles, footer]) => {
      ...
      for (let queryType in QueryType) {
        ...
        maxQueryWords[qt] = serverConf.freqDB[qt]
          ? serverConf.freqDB[qt].maxQueryWords
          : 1;
      }
      if (!conf.layouts[domain]) {
        const avail = Object.keys(conf.layouts || {});
        throw new Error(
          `Missing layouts configuration for domain "${domain}" (available: ${avail.map((x) => `"${x}"`).join(", ")})`,
        );
      }
      return {
        ...
        tiles:
          typeof conf.tiles === "string" || isTileDBConf(conf.tiles)
            ...
            : pipe(
                conf.tiles[domain],
                Dict.map((item) => ({
                  waitForTimeoutSecs: DEFAULT_WAIT_FOR_OTHER_TILES,
                  ...item,
                })),
              ),
...
export function importQueryRequest({
  ...
  return new Observable<UserConf>((observer) => {
    try {
      const queries = fetchUrlParamArray(
        req,
        "query",
        queryType === QueryType.CMP_QUERY ? 2 : 1,
      );
      const queryDomain = fetchUrlParamArray(
        req,
        "domain",
        queryType === QueryType.TRANSLAT_QUERY ? 2 : 1,
      );
      const layouts = services.clientConf.layouts;
      if (answerMode && typeof layouts !== "string") {
...
function testGroupedAuth(
  ...
  return rxOf(...items).pipe(
    concatMap((item) =>
      serverHttpRequest<any>({
        url: item.preflightUrl,
        method: HTTP.Method.GET,
        cookies: req.cookies,
      }).pipe(
        map((resp) => ({ authorized: true, conf: item })),
        catchError((err) => {
          return rxOf({ authorized: false, conf: item });
        }),
      ),
    ),
    concatMap(({ authorized, conf }) => {
      return authorized
        ? rxOf(true)
...
function markMatch(
  ...
  if (userQuery.lemma && !List.empty(userQuery.pos)) {
    const srch = List.findIndex(
      (x) =>
        matchesPos(x, posAttr, userQuery.pos) && userQuery.lemma === x.lemma,
      availMatches,
    );
    if (srch > -1) {
  ...
  importQueryRequest({
      ...
      concatMap(([runtimeConf, userConf, layoutManager]) =>
        forkJoin({
          ...
          userConf: new Observable<UserConf>((observer) => {
            if (
              userConf.queryType === QueryType.TRANSLAT_QUERY &&
              userConf.query1Domain === userConf.query2Domain
            ) {
              userConf.error = [
                400,
                appServices.translate(
                  "global__src_and_dst_domains_must_be_different",
                ),
              ];
            }
          ...
          qMatchesEachQuery: rxOf(
            ...
            concatMap((v) => v),
            reduce(
              (acc: Array<Array<QueryMatch>>, curr) => acc.concat([curr]),
              [],
            ),
          ),
        }),
    ...
    .subscribe({
      next: ({
        ...
        const queryMatchesExtended = List.map((queryMatches, queryIdx) => {
        ...
        res.send(
          renderResult({
            ...
            htmlTitle: services.clientConf.htmlTitle
              ? appServices.importExternalMessage(services.clientConf.htmlTitle)
              : undefined,
