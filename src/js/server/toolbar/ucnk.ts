...
export class UCNKToolbar implements IToolbarProvider {
    ...
    get(
        ...
        List.forEach((arg) => {
            data.set(
                arg.substring(UCNKToolbar.PREFIX.length),
                cookies[arg] || ''
            );
        ...
        return serverHttpRequest<ToolbarResponse>({
            ...
            catchError((err: Error) => {
                throw err instanceof ServerHTTPRequestError
                    ? new Error(
                          `Toolbar loading failed with error: ${err.statusText} (code ${err.status})`
                      )
                    : err;
            }),
            map<ToolbarResponse, HostPageEnv>((response) => ({
