...
export function init<T>(
    ...
    const Word: React.FC<{
        ...
        const handleMouseMove = (e: React.MouseEvent) => {
            props.onMouseMove(
                Math.max(e.pageX + 20, 0),
                Math.max(e.pageY - 50, 0),
                props.rect.data
            );
        };
        ...
        return (
            ...
            >
                <rect
                    ...
                    opacity={
                        props.underline &&
                        props.rect.data.text === props.selectedText
                            ? 1
                            : 0
                    }
                />
                ...
                <text
                    x={props.rect.x + props.rect.w / 2}
                    y={
                        props.rect.y +
                        props.rect.h / 2 +
                        props.rect.fontSize / 4
                    }
                    fill={
                        props.underline &&
                        props.rect.data.text === props.selectedText
    ...
    class Tooltip extends React.PureComponent<{
        ...
        private calcXPos(): number {
            return this.elmRef.current
                ? Math.max(
                      0,
                      this.props.x -
                          this.elmRef.current.getBoundingClientRect().width -
                          5
                  )
                : this.props.x;
        }
        ...
        private calcYPos(): number {
            return this.elmRef.current
                ? this.props.y +
                      this.elmRef.current.getBoundingClientRect().height +
                      5
                : this.props.y;
        }
    ...
    const WordCloud: React.FC<WordCloudProps<T>> = (props) => {
        const chartContainer: React.RefObject<HTMLDivElement> =
            React.useRef(null);
        ...
        return (
            <div ref={chartContainer} style={style}>
                <Tooltip
                    x={currState.tooltipPos[0]}
                    y={currState.tooltipPos[1]}
                    data={
                        currState.activeItem ? currState.activeItem.tooltip : []
                    }
                />
                <svg
                    width="100%"
