...
const groupData = (
    ...
    { [area: string]: Array<TargetDataRow> },
    { [area: string]: number },
    { [area: string]: number },
] => {
    ...
    const groupedIpmNorms = pipe(
        ...
        List.map(([area, data]) =>
            tuple(
                area,
                data.reduce((acc, curr) => acc + curr.ipm, 0)
            )
        ),
        Dict.fromEntries()
    );
    const groupedAreaAbsFreqs = pipe(
        ...
        List.map(([area, data]) =>
            tuple(
                area,
                data.reduce((acc, curr) => acc + curr.freq, 0)
            )
        ),
        Dict.fromEntries()
    );
    return [
        Dict.fromEntries(groupedData),
        groupedIpmNorms,
        groupedAreaAbsFreqs,
    ];
};
...
export function init(
    ...
    const createSVGPieChart = (
        parent: Element,
        areaIpmNorm: number,
        areaData: Array<TargetDataRow>,
        radius: number,
        currentQueryMatches: Array<QueryMatch>
    ): SVGElement => {
        ...
        List.forEach((row) => {
            ...
            createSVGElement(pieSlices, 'path', {
                d:
                    ipmFrac === 1
    ...
    const createSVGLegend = (
        ...
        currentQueryMatches.forEach((match, index) => {
            ...
            createSVGElement(legendItem, 'rect', {
                x: '0',
                y: '0',
                width: '100',
                height: '100',
            ...
            const text = createSVGElement(legendItem, 'text', {
    ...
    const DataTable: React.FC<{
        ...
        const [groupedAreaData, groupedAreaIpmNorms, groupedAreaAbsFreqs] =
            ...
            <table className="DataTable data cnc-table">
                <thead>
                    <tr>
                        ...
                        <th colSpan={2}>
                            {ut.translate(
                                'geolocations__cmp_table_heading_total_occurrence'
                            )}
                        </th>
                        {List.map(
                            (targetData, target) => (
                                <th key={target} colSpan={2}>
                                    {ut.translate(
                                        'geolocations__cmp_table_heading_occurrence_of_{word}',
                                        {
                                            word: props.queryMatches[target]
                                                .word,
                                        }
                                    )}
                                </th>
                    ...
                    <tr>
                        <th key={`totalIpm`}>
                            {ut.translate(
                                'geolocations__cmp_table_heading_freq_rel'
                            )}
                        </th>
                        <th key={`totalAbs`}>
                            {ut.translate(
                                'geolocations__cmp_table_heading_freq_abs'
                            )}
                        </th>
                ...
                <tbody>
                    {pipe(
                        groupedAreaData,
                        Dict.toEntries(),
                        List.sorted(([area1], [area2]) =>
                            area1.localeCompare(area2)
                        ),
                        List.map(([area, rows]) => (
                            <tr key={area}>
                                <td key={area}>{area}</td>
                                <td key={`${area}Ipm`} className="num">
                                    {ut.formatNumber(
                                        groupedAreaIpmNorms[area],
                                        2
                                    )}
                                </td>
                                <td key={`${area}Abs`} className="num">
                                    {ut.formatNumber(
                                        groupedAreaAbsFreqs[area],
                                        2
                                    )}
                                </td>
                                {List.map((_, target) => {
    ...
    const drawLabels = (
        ...
        const [groupedAreaData, groupedAreaIpmNorms, groupedAreaAbsFreqs] =
            groupData(data);
        const maxIpmNorm = Math.max(
            ...Dict.toEntries(groupedAreaIpmNorms).map(([, v]) => v)
        );
        const minIpmNorm = Math.min(
            ...Dict.toEntries(groupedAreaIpmNorms).map(([, v]) => v)
        );
        ...
        Dict.forEach((areaIdent, areaName) => {
            ...
            if (element) {
                ...
                const notEnoughData =
                    !areaData ||
                    groupedAreaAbsFreqs[areaName] < frequencyDisplayLimit;

                if (notEnoughData) {
                    areaIpmNorm = 0;
                    scale = 0.9;

                    pieChart = createSVGEmptyCircle(element, 150);
                } else {
                    areaIpmNorm = groupedAreaIpmNorms[areaName];
                    scale =
                        0.9 +
                        (areaIpmNorm - minIpmNorm) /
                            (maxIpmNorm - minIpmNorm) /
                            5;
        ...
        // insert legend
        const legendHolder = document.querySelector(
            `section#tile-${tileId} .svg-graph-p .legend-g`
        );
