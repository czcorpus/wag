...
export function init(
    ...
    const SimpleBar: React.FC<{
        ...
        const tooltipVals = {
            ...
            [ut.translate('treqsubsets__perc')]: [
                { value: props.perc, unit: '%' },
            ],
        };
        ...
        return (
            <S.SimpleBar
                style={{
                    height: `${Math.round(props.width / 3.333)}px`,
            ...
            <S.ChartLikeTable>
                <thead>
                    <tr>
                        ...
                        {props.subsets.map((v, i) => (
                            <th key={v.label} className="package">
                                {v.label}
                            </th>
                        ))}
                    </tr>
                ...
                <tbody>
                    {flipRowColMapper(
                        ...
                        (row) => (
                            <tr
                                key={`${row.heading}:${row.idx}`}
                                className={
                                    props.highlightedRowIdx === row.idx
                                        ? 'highlighted'
                                        : null
                                }
                            >
            ...
            <S.AltViewTable className="data">
                <thead>
                    ...
                    <tr className="absrel bottom-grouped">
                        <th />
                        {props.subsets.map((v, i) => (
                            <React.Fragment key={`A:${v.label}`}>
                                <th>abs.</th>
                                <th className="separ">rel.</th>
                            </React.Fragment>
                        ))}
                    </tr>
                    <tr className="separ">
                        <td colSpan={props.subsets.length * 2} />
                    </tr>
                </thead>
                <tbody>
                    {flipRowColMapper(
    ...
    > {
        ...
        render() {
            return (
                <div>
                    <globalComponents.ElementTooltip
