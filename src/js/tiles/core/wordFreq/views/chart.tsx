...
export function init(
    ...
    const Chart: React.FC<{
        ...
        const dataLimitIdx =
            List.findIndex(
                (v) => v.ipm > queryMatches[queryMatches.length - 1].ipm,
                dataAll
            ) + 1;
        const bandTickLimitIdx =
            List.findIndex(
                (v) => v > queryMatches[queryMatches.length - 1].ipm,
                fBands
            ) + 1;
        ...
        const tickFmt = (value: number, index: number) =>
        ...
        return (
            <globalCompontents.ResponsiveWrapper
                ...
                render={(width: number, height: number) =>
                    isClient ? (
                        <LineChart
                            id={`word-freq-chart-${props.tileName}`}
                            data={data}
                            width={width}
                            height={height}
                        >
                            ...
                            <XAxis
                                dataKey="flevel"
                                type="number"
                                domain={[1, xTicks[xTicks.length - 1]]}
                                ticks={xTicks}
                                tickFormatter={tickFmt}
                                tick={{ fill: theme.chartTextColor }}
                            >
                                <Label
                                    value={ut.translate('wordfreq__freq_bands')}
                                    offset={0}
                            ...
                            <YAxis
                                dataKey="ipm"
                                type="number"
                                ticks={List.slice(0, yLimit, fBands)}
                                tick={{ fill: theme.chartTextColor }}
                            >
                                <Label
                                    value={ut.translate('wordfreq__ipm')}
                                    offset={5}
                                    angle={-90}
                            ...
                            <Line
                                ...
                                dot={({ cx, cy, stroke, payload, value }) => (
                                    <LineDot
                                        ...
                                        value={value}
                                        active={
                                            payload.ident === props.activeIdent
                                        }
                                    />
                                )}
                            />
