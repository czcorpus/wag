...
const calcIPM = (v: DataItemWithWCI, domainSize: number) =>
    Math.round((v.freq / domainSize) * 1e6 * 100) / 100;
...
export class TimeDistribModel extends StatelessModel<TimeDistribModelState> {
            ...
            (state, action) => {
                if (action.payload.data) {
                    state.data[action.payload.queryId] = this.mergeChunks(
                        action.payload.overwritePrevious
                            ? []
                            : state.data[action.payload.queryId],
                        action.payload.data,
                        state.alphaLevel
                    );
                    state.mainBacklinks[action.payload.queryId] =
                        this.api.getBacklink(action.payload.queryId);
                } else if (action.payload.dataCmp) {
                    state.dataCmp = this.mergeChunks(
                        action.payload.overwritePrevious ? [] : state.dataCmp,
                ...
                if (action.payload.wordMainLabel) {
                    state.wordMainLabels[action.payload.queryId] =
                        action.payload.wordMainLabel;
                }
            }
        );
        ...
        this.addActionHandler<typeof Actions.ChangeTimeWindow>(
            ...
            (state, action) => {
                if (action.payload.tileId === this.tileId) {
                    state.averagingYears = action.payload.value;
                }
            }
        );
    ...
    private mergeChunks(
        currData: Array<DataItemWithWCI>,
        newChunk: Array<DataItemWithWCI>,
        alphaLevel: Maths.AlphaLevel
        ...
        return pipe(
            ...
            List.foldl(
                ...
                Dict.fromEntries(
                    List.map(
                        (v) => [v.datetime, v] as [string, DataItemWithWCI],
                        currData
                    )
                )
            ),
        ...
        response
            .pipe(
                ...
                reduce(
                    (acc, payload) =>
                        acc &&
                        (payload.dataCmp || payload.data || []).length === 0,
                    true
                )
            )
            .subscribe({
