...
export function init(
    ...
    function transformData(
        ...
        return pipe(
            data,
            List.flatMap((v, i) =>
                v ? v.map<[SourceMappedDataRow, number]>((v) => [v, i]) : []
            ),
            ...
            <table className="data" id={`${props.tileId}-download-table`}>
                <thead>
                    {props.queryMatches.length > 1 ? (
                        <tr>
                            <th />
                            {props.queryMatches.map((value, index) => (
                                <th
                                    key={value.lemma}
                                    colSpan={2}
                                >{`[${index + 1}] ${value.word}`}</th>
                            ))}
                        </tr>
                    ) : null}
                    <tr>
                        ...
                        {props.queryMatches.map((lemma, idx) => (
                            <React.Fragment key={lemma.lemma}>
                                <th>
                                    {ut.translate('mergeCorpFreq__abs_freq')}
                                </th>
                                <th>
                        ...
                        <tr key={`${i}:${row.name}`}>
                            ...
                            {props.queryMatches.map((v, index) => (
                                <React.Fragment key={v.lemma}>
                                    <td className="num">
                                        {ut.formatNumber(row.freq[index])}
                                    </td>
                                    <td className="num">
                                        {ut.formatNumber(row.ipm[index])}
                                    </td>
                                </React.Fragment>
                            ))}
                        </tr>
    ...
    const CustomLabel: React.FC<{
        ...
        typeof props.onClick === 'function' ? (
            <g
                transform={`translate(${props.x + props.width + 10}, ${props.y + 1}) scale(2)`}
            >
                <SVGLink />
            </g>
        ) : null;
    ...
    const Chart: React.FC<{
        ...
        const maxLabelLength = List.maxItem<string>(
            ...
            props.isMobile
                ? List.flatMap(
                      (item) => [
                          Strings.shortenText(item.name, CHART_LABEL_MAX_LEN),
                      ],
                      transformedData
                  )
        ...
        const colorFn =
            queries > 1
                ? (idx: number) =>
                      theme.cmpCategoryColor(idx, props.queryMatches.length)
                : (idx: number) =>
                      transformedData[idx].uniqueColor
                          ? theme.cmpCategoryColor(
                                idx,
                                List.size(transformedData)
                            )
            ...
            return (
                <g transform={`translate(${x},${y})`}>
                    {data.isClickable && data.freq.length === 1 ? (
                        <a
                            onClick={props.onBarClick(payload.index, 0)}
