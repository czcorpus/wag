...
export class MergeFreqsApi
    implements ResourceApi<Array<MQueryFreqArgs>, Array<SingleFreqResult>>
{
    ...
    constructor(
        ...
        this.freqApi = new MQueryFreqDistribAPI(
            this.apiURL,
            this.apiServices,
            this.backlinkConf
        );
    ...
    private mkRequest(
        ...
        tileId: number,
        queryIdx: number,
        args: Array<MQueryFreqArgs | null>
    ): Observable<HTTPResponse> {
        return streaming
            .registerTileRequest<HTTPResponse>({
                tileId,
                method: HTTP.Method.POST,
                url: this.isNoMatchArgs(args)
                    ? ''
                ...
                body: {
                    urls: this.isNoMatchArgs(args)
                        ...
                        : List.map(
                              (arg) =>
                                  urlJoin(this.apiURL, arg.path, arg.corpname) +
                                  '?' +
                                  this.prepareArgs(arg),
                              args
                          ),
                },
    ...
    call(
        ...
        tileId: number,
        queryIdx: number,
        args: Array<MQueryFreqArgs>
    ): Observable<Array<SingleFreqResult>> {
