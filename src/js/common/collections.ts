...
export function dictFromList<T>(items:Array<[string, T]>):{[key:string]:T} {
...
export function dictToList<T>(data:{[key:string]:T}):Array<[string, T]> {
...
export function dictForEach<T>(data:{[key:string]:T}, fn:(v:T, k:string)=>void):void {
    for (let k in data) {
        fn(data[k], k);
    }
}

export function dictMap<T, U>(data:{[key:string]:T}, mapper:(v:T, k:string)=>U):{[key:string]:U} {
 ...
 */
export function dictHasValue<T>(data:{[key:string]:T}, v:T):boolean {
    for (let k in data) {
        if (data[k] === v) {
            return true;
        }
...
export function dictHasKey<T>(data:{[key:string]:T}, k:string):boolean {
    return data[k] !== undefined;
}

export function mapDictEntries<T, U>(data:{[key:string]:T}, mapper:(entry:[string, T])=>U):Array<U> {
    const ans:Array<U> = [];
    for (let k in data) {
...
export function flatMapList<T, U>(data:Array<T>, mapper:(v:T, i:number) => Array<U>):Array<U> {
    return data.reduce(
...
export function sortListBy<T>(data:Array<T>, map:(v:T) => number):Array<T> {
    return data.sort((v1, v2) => map(v1) - map(v2));
}

export function findListRange<T>(data:Array<T>, cmp:(v1:T, v2:T) => number):[T, T] {
    let min:T = data[0];
    let max:T = data[0];
    data.forEach(v => {
        if (cmp(v, min) < 0) {
...
export function listToObject<T>(data:Array<T>):{[key:string]:T} {
    const ans:{[key:string]:T} = {};
    data.forEach((v, i) => {
        ans[i.toFixed()] = v;
    });
...
export function listMaxItem<T>(data:Array<T>, mapper:(v:T)=>number):T {
    let max = data[0];
