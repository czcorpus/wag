...
export function loadTileData<T>(tileId:string, queryIdx?:number):T {
  const tileData = previewTileData[tileId];
  const qIdx = (queryIdx !== undefined) && (tileData.length > queryIdx) ? queryIdx : 0;
  return tileData[qIdx] as T;
}

const previewTileData:{[key:string]:Array<any>} = {
    PREVIEW__wordFreq: [
        {
            "matches": [
              {
