...
export class QueryFormModel extends StatelessModel<QueryFormModelState> {
    ...
    constructor(
        ...
        initialState: QueryFormModelState
    ) {
        super(dispatcher, initialState);
        this.appServices = appServices;
        this.queryValidator = new QueryValidator(this.appServices);

        this.addActionHandler(Actions.ChangeQueryInput, (state, action) => {
            state.errors = [];
            state.queries[action.payload.queryIdx] = Forms.updateFormInput(
                state.queries[action.payload.queryIdx],
                { value: action.payload.value, isValid: true }
            );
        });
                ...
                if (state.queries.length >= state.maxCmpQueries) {
                    this.appServices.showMessage(
                        ...
                        this.appServices.translate(
                            'global__maximum_limit_of_compared_queries'
                        )
                    );
                }
            }
        ...
        this.addActionHandler(Actions.HideQueryMatchModal, (state, action) => {
        ...
        this.addActionHandler(
            ...
            (state, action) => {
                state.modalSelections[action.payload.queryIdx] =
                    action.payload.variantIdx;
            }
        );
    ...
    private normalizeQueries(state: QueryFormModelState): void {
        state.queries = List.map(
            (formQuery) =>
                Forms.updateFormInput(formQuery, {
    ...
    private validateNthQuery(state: QueryFormModelState, idx: number): boolean {
        ...
        state.queries[idx] = Forms.updateFormInput(state.queries[idx], {
