...
export class QueryValidator {
    ...
    validateQuery(q:string, maxNgramSize:number):Array<Error> {
        ...
        if (!this.queryCheckRegexp.exec(q) && !this.hyphenChars.exec(q)) {
            ans.push(new Error(this.appServices.translate('global__query_contains_unsupported_chars')));
        }
        const spaceMatch = q.match(/\s/g);
        const spaceCount = spaceMatch ? spaceMatch.length : 0;
        if (spaceCount+1 > maxNgramSize) {
            ans.push(new Error(this.appServices.translate('global__query_words_exceeded_{num_words}',
