...
import {
  ...
  ConcViewArgs,
  FilterServerArgs,
  QuickFilterRequestArgs,
} from "../../types";
import { Backlink } from "../../../../../page/tile";

export class ConcApi
  implements
    IConcordanceApi<
      ConcQueryArgs | ConcViewArgs | FilterServerArgs | QuickFilterRequestArgs
    >,
    WebDelegateApi
{
  ...
  stateToArgs(
    ...
    lvar: QueryMatch | null,
    lvarIdx: number,
    otherLangCql: string,
  ): ConcQueryArgs | ConcViewArgs {
    if (lvar) {
  ...
  private createActionUrl(
    ...
    switch (args.type) {
      ...
      case "concQueryArgs":
        return tuple(
          URL.join(this.apiURL, "query_submit"),
          "application/json",
          args,
        );
  ...
  call(
    ...
    const corpname =
      args.type === "concQueryArgs" ? args.queries[0].corpname : args.corpname;
    ...
    return (
      ...
      map((data) => ({
        ...
        kwicNumTokens:
          data.Lines.length > 0 ? parseInt(data.Lines[0].kwiclen) : 1,
        concsize: data.concsize,
        arf: data.result_arf,
        ipm: data.result_relative_freq,
        query:
          args.type === "concQueryArgs" ? List.head(args.queries).query : null, // TODO filter type?
