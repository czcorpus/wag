...
import { ajax$, cachedAjax$, encodeURLParameters } from '../../../../../page/ajax';
...
export class ConcTokenApi implements IConcordanceApi<ConcQueryArgs|ConcViewArgs|FilterServerArgs|QuickFilterRequestArgs>, WebDelegateApi {
    ...
    _call(args:ConcQueryArgs|ConcViewArgs|FilterServerArgs|QuickFilterRequestArgs):Observable<ConcResponse> {
    ...
    call(args:ConcQueryArgs|ConcViewArgs|FilterServerArgs|QuickFilterRequestArgs):Observable<ConcResponse> {
        return this._call(args).pipe(
            catchError((err, _) => {
                if (err.status === HTTP.Status.Forbidden) {
                    return this.authenticate().pipe(
                        concatMap(_ => this._call(args))
                    );
                } else {
                    throwError(() => err);
                }
