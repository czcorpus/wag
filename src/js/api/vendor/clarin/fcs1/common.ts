...
export function importRecord(node: XMLNode, lineNum: number): Line {
    const recordData = node.findChildRecursive(
        (item) => item.name === 'hits:Result',
        new XMLNode()
    );
    let key = 'left';
...
function importDiagnostics(root: XMLNode): string | undefined {
    ...
    if (srch) {
        ...
        if (srch2) {
            const msgElm = srch2.findChild(
                (v) => v.name === 'diag:message',
                new XMLNode()
            );
            const detailsElm = srch2.findChild(
                (v) => v.name === 'diag:details',
                new XMLNode()
            );
...
export function importResponse(
    ...
    const respNode = root.findChild(
        (v) => v.name === 'sru:searchRetrieveResponse'
    );
    if (respNode) {
        const numRecNode = respNode.findChild(
            (v) => v.name === 'sru:numberOfRecords',
            new XMLNode()
        );
        ...
        const recordsRootNode = respNode.findChild(
            (v) => v.name === 'sru:records'
        );
        ans.lines = recordsRootNode
            ? recordsRootNode
                  .findAllChildren((v) => v.name === 'sru:record')
                  .map(importRecord)
            : [];
    } else {
