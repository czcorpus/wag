...
export class MQueryFreqDistribAPI
    implements ResourceApi<MQueryFreqArgs, APIResponse>
{
        ...
        return ajax$<{ conc_persistence_op_id: string }>(
            ...
            concatMap((resp) => {
                ...
                url.searchParams.set(
                    'freq_type',
                    args.path === 'freqs' ? 'tokens' : 'text-types'
                );
                url.searchParams.set(
                    'flimit',
                    args.queryArgs.flimit.toString()
                );
