...
class TreqAPICaller {
  ...
  constructor(
    ...
    this.descI18n = {
      ...
      "en-US":
        "A project managed by the Institute of the Czech National Corpus",
    };
  }
  ...
  getSourceDescription(
    ...
    return rxOf({
      ...
      citationInfo: {
        ...
        otherBibliography:
          '<a target="_blank" rel="noopener" href="https://intercorp.korpus.cz/">intercorp.korpus.cz</a>',
      },
    });
  }

  private mergeByLowercase(
    lines: Array<WordTranslation>,
  ): Array<WordTranslation> {
    return Object.values<WordTranslation>(
      lines.reduce((acc, curr) => {
        if (!(curr.firstTranslatLc in acc)) {
          ...
          curr.translations.forEach((variant) => {
            if (
              acc[curr.firstTranslatLc].translations.indexOf(variant) === -1
            ) {
  ...
  call(args: RequestArgs): Observable<TranslationResponse> {
    const headers = {
      ...this.appServices.getApiHeaders(this.apiURL),
      "X-Is-Web-App": "1",
      ...this.appServices.getApiReportingHeaders(),
    };
...
export class TreqAPI
  extends TreqAPICaller
  implements TranslationAPI<RequestArgs, PageArgs>
{
  constructor(
    cache: IAsyncKeyValueStore,
    apiURL: string,
  ...
  stateToArgs(
    state: TranslationsModelState<PageArgs>,
    query: string,
  ): RequestArgs {
    return {
      from: state.domain1,
...
export class TreqSubsetsAPI
  extends TreqAPICaller
  implements TranslationSubsetsAPI<RequestArgs>
{
  stateToArgs(
