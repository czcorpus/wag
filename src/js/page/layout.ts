...
function layoutIsLayoutOfQueryTypeTranslat(
    t: LayoutCore
): t is LayoutOfQueryTypeTranslat {
...
function importLayout(
    gc: LayoutConfigCommon | undefined,
              ...
              groups: pipe(
                  ...
                  List.map((group) => {
                      if (itemIsGroupConf(group)) {
                          ...
                          return {
                              groupLabel: appServices.importExternalMessage(
                                  group.groupLabel
                              ),
                              groupDescURL:
                                  appServices.externalMessageIsDefined(descUrl)
...
export class LayoutManager {
    ...
    private validateLayout(): void {
        pipe(
            ...
            List.forEach((item) => {
                console.error(
                    `Invalid layout configuration for group ${item.group} at position ${item.idx}`
                );
            })
        );
    }
    ...
    isInCurrentLayout(tileId: number | string): boolean {
        const tTileId =
            typeof tileId === 'string' ? this.getTileNumber(tileId) : tileId;
        return pipe(
            this.getLayout().groups,
            List.flatMap((v) => v.tiles),
