...
function isOtherTileRequest(
    t: TileRequest | OtherTileRequest
): t is OtherTileRequest {
...
function normalizeRequest<T extends TileRequest | OtherTileRequest>(req: T): T {
    if (isOtherTileRequest(req)) {
        return {
            ...
            otherQueryIdx:
                typeof req.otherTileQueryIdx === 'number'
                    ? req.otherTileQueryIdx
                    : 0,
        };
    }
    return {
            ...
            filter((response: EventItem<T>) => {
                if (isOtherTileRequest(normEntry)) {
                    return (
                        ...
                        response.queryIdx === normEntry.otherTileQueryIdx
                    );
                } else {
                    return (
                        response.tileId === normEntry.tileId &&
                        response.queryIdx === normEntry.queryIdx
                    );
                }
            }),
            map((response) => {
...
export class DataStreamingPreview implements IDataStreaming {
    registerTileRequest<T>(
        ...
        const tileId = splitUrl[1].endsWith('2')
            ? splitUrl[1].substring(0, entry.url.length - 1)
            : splitUrl[1];
        return new Observable<T>((observer) => {
            import(
                /* webpackChunkName: "previewData" */ '../conf/previewData.js'
            ).then((module) => {
                ...
                if (tileData) {
                    ...
                    observer.error(
                        new Error(
                            `No preview data for tile ${tileId} and query index ${entry.queryIdx}`
                        )
                    );
                }
                observer.complete();
