...
function initTelemetry(
  ...
  if (
    config.telemetry &&
    Math.random() < config.telemetry.participationProbability
  ) {
    merge(
      ...
      rxOf(1, 1.2, 1.4, 1.6, 1.8, 2.0, 2.3, 2.6, 3, 4.0, 6, 10).pipe(
        concatMap((v) =>
          interval(config.telemetry.sendIntervalSecs * v * 1000).pipe(take(1)),
        ),
      ),
    )
      .pipe(
        scan<
          number | TelemetryAction,
          { toDispatch: Array<TelemetryAction>; buffer: Array<TelemetryAction> }
        >(
          (acc, curr) =>
            typeof curr === "number"
              ? {
...
export function initClient(
  ...
  const appServices = new AppServices({
    ...
    apiReportingHeaders: config.apiReporting
      ? {
          [config.apiReporting.headerName]: config.apiReporting.headerValue,
        }
      : {},
  });
