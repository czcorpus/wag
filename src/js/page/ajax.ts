...
const exportValue = (v: string | number | boolean) =>
        ...
        if (obj[p] !== undefined) {
            ...
            if (Array.isArray(val)) {
                if (p.endsWith('[i]')) {
                    val.forEach((item, i) => {
                        ans.push(
                            encodeURIComponent(p.replace('[i]', `[${i}]`)) +
                                '=' +
                                exportValue(item)
                        );
                    });
                    ...
                    val.forEach((item) => {
                        ans.push(
                            encodeURIComponent(p) + '=' + exportValue(item)
                        );
                    });
                }
...
export function encodeURLParameters(
    params: MultiDict | Array<[string, any]>
): string {
    const exportValue = (v: string) =>
        v === null || v === undefined ? '' : encodeURIComponent(v);

    return (MultiDict.isMultiDict(params) ? params.items() : params)
